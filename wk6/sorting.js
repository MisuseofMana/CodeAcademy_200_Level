//array of pi's decimal digits as strings
...
const stringsToNumbs = (numbers) => {
    let piNumbersArr = numbers.map(num => Number(num));
    return piNumbersArr;
}

//new variable set to the number values of the string array strNums passed through the stringsToNums() function
console.log(`strNums strings converted to Numbers data type`);
console.log(stringsToNumbs(strNums));

//store Number array as a variable
let piNumbs = stringsToNumbs(strNums);
...
const sumEvens = (arr) => {
  ...
  let sumOfEvenArr = evenArr.reduce((accumulator, currentValue) => accumulator + currentValue);
  return sumOfEvenArr;
}

const piEvenAdded = sumEvens(stringsToNumbs(strNums));
console.log(`pi's even decimal digits added together to the 1000th decimal place`);
console.log(piEvenAdded);
...
const atxIdx = (arr) => {
  let indexArray = arr.map((number, index) => number + index);
  return indexArray.findIndex((number) => number == 512);
}

console.log(`the index of the first number when added to it's index = 512`);
console.log(atxIdx(piNumbs));
...
const weatherStates = (arr) => {
  ...
  for (items in arr) {
    weatherStateNames.push(arr[items].weather_state_name);
  }

  return weatherStateNames.filter((item, index) => weatherStateNames.indexOf(item) === index);

}
console.log(`Should return below ['Light Cloud, 'Heavy Cloud', 'Showers']`);
console.log(weatherStates(weather))

//find the id of the object in weather that has a min_temp of 15.915
const idealTemp = (arr) => {
  for (item in arr) {
    if(arr[item].min_temp ===  15.915){
      return arr[item].id;
    }
  }
}

console.log(`Should return below 6696130918219776`);
